...
def ja_categolize(line, dic):    #日本語のデータを、日付、ID、タイトル、内容、タイトルのbag of word、内容のbag of wordを返す。
    if u"＼Ｃ０＼" in line:
...
def en_categolize(line, dic, i):
    if i % 3 == 0:
        ...
        return dic
def make_en_bag(sentence):    #英語のbag of wordの生成
    lis = []
    koyu_lis = []
    num_lis = []
...
def make_dict():
    ...
    f.close()
    return word_dict
def search_best_sent(ja_art, en_art, word_dict):
    ...
    for i in range(len(en_art)):
	    ...
	    ja_day = ja_line.get("day")
            ...
            if ja_year < en_year:
                ...
                break
            co_score = (float(co_score)/(k-z+1))*100
            if not len(en_num_content)+len(en_koyu_content)-y == 0:
                ex_co_score = (float(ex_co_score)/(len(en_num_content)+len(en_koyu_content)-y))*100
            else:
                ex_co_score = 0
            if len(ex_best_sent) < 20:
                a = []
            ...
            elif len(ex_best_sent) == 20:
                if ex_best_sent[0][0] < ex_co_score:
                    a = []
                    a.append(co_score)
                    a.append(ja_line["raw_title"])
                    a.append(ja_line["raw_content"])
